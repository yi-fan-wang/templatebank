"""Generate or densify a stochastic template bank (v4).
...
v4 fixes the worker-pool lifecycle without changing proposal drawing, waveform
generation, match evaluation, or acceptance ordering:
- Worker processes reset the parent-only graceful SIGTERM handler.  In v3,
  workers inherited that handler, so Pool termination produced a signal storm
  and could wait forever for workers that refused to exit.
- Successful pools use close()+join(), not Pool.__exit__()'s terminate().
- Per-round screening and dedup wall times are logged for utilization audits.
...
@contextmanager
def worker_pool(processes, initializer=None, initargs=()):
    """Close successful pools cleanly; terminate only after an exception."""
...
class TriangleBank(object):
    ...
    def check_params(self, params):
        ...
        dedup_seconds = time.monotonic() - dedup_start
        ...
        logging.info("round timing: screen %.1f s with %i workers; "
                     "dedup+insert %.1f s", screen_seconds,
                     self.nprocesses, dedup_seconds)
    ...
    def add_existing_bank(self, params, seen):
        ...
        with worker_pool(
                self.nprocesses,
                initializer=_waveform_worker_init,
        ) as pool:
...
def _waveform_worker_init():
    _reset_worker_signals()


def _reset_worker_signals():
    """Keep graceful checkpoint signals parent-only."""
